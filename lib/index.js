...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
      ...
      if (parts.length === 2) {
        ...
        if (/^Bearer$/i.test(scheme)) {
          ...
          if (credentialsRequired) {
            return res(new UnauthorizedError('credentials_bad_scheme', { message: 'Format is Authorization: Bearer [token]' }));
          } else {
            return next();
          }
        }
      } else {
        return res(new UnauthorizedError('credentials_bad_format', { message: 'Format is Authorization: Bearer [token]' }));
      }
    }

    if (!token) {
      if (credentialsRequired) {
        return res(new UnauthorizedError('credentials_required', { message: 'No authorization token was found' }));
      } else {
        return next();
      }
    }
    ...
    try {
      ...
      return res(new UnauthorizedError('invalid_token', err));
    }
